    ...
    : QListView(parent)
{
    ...
    connect(m_scrollBar, &QScrollBar::valueChanged,
            this, [&](int value) {
                qDebug()<<Q_FUNC_INFO<<" QScrollBar::valueChanged "<<value;
...
{
    qDebug()<<Q_FUNC_INFO<<" row count "<<model->rowCount();
...
void RKListView::updateScrollBar()
{
    if (!this->model() || !m_defaultItemSize.isValid()) {
        qDebug()<<Q_FUNC_INFO<<"no model or invalid defaultItemSize";
        return;
    }
    ...
    qDebug()<<Q_FUNC_INFO<<" this size "<<size
