...
{
    if (!m_proxy) {
        qWarning()<<Q_FUNC_INFO<<"proxy is nullptr !!!";
        return;
    }
    if (!uri.isValid()) {
        qWarning()<<Q_FUNC_INFO<<"Invalid uri : "<<uri;
...

    {
        ...
        if (url.isValid() && url == uri) {
            qDebug()<<Q_FUNC_INFO<<"current QModelIndex ["<<idx<<"] for url ["<<uri<<"] is in ignore map";
            return;
        }
    }
    ...
    connect(reply, &QCNetworkAsyncReply::finished,
            this, [&, idx, uri, reply](){
                if (uri != m_urlMap.value(idx)) {
                    qWarning()<<Q_FUNC_INFO<<" url not same ["<<uri<<"] <==> ["<<m_urlMap.value(idx)<<"]";
                }

                if (reply != m_replyMap.value(idx)) {
                    qWarning()<<Q_FUNC_INFO<<" reply ptr not same ["<<reply<<"] <==> ["<<m_replyMap.value(idx)<<"]";
                }
                ...
                if (error == NetworkNoError) {
                    ...
                    if (QFile::exists(file)) {
                        ...
                        if (!f.open(QIODevice::WriteOnly)) {
                            qDebug()<<Q_FUNC_INFO<<"Open file error: "<<file;
                            m_proxy->onImageFile(QString(), idx);
                            return;
                        }
                        f.write(data);
                    ...
                    m_ignoreMap.insert(idx, uri);
                    qDebug()<<Q_FUNC_INFO<<"download from uri ["<<uri<<"] error: "<<reply->errorString();
                }
            });
    reply->perform();
}
