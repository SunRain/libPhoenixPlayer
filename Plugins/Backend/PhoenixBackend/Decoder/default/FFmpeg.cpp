...
bool FFmpeg::initialize(MediaResource *res)
{
    ...
    if (avcodec_parameters_to_context(c, ic->streams[audioIndex]->codecpar) < 0) {
        qWarning()<<Q_FUNC_INFO<<"avcodec_parameters_to_context error";
        close();
        return false;
    }
#else
    ...
    qDebug()<<Q_FUNC_INFO<<" c->channels "<<c->channels;
...
//    audio_st = ic->streams[audio_stream_id];
    ...
    m_parameter = AudioParameters(c->sample_rate, /*m_channels*/ChannelMap(m_channels), m_audioFormat);

    qDebug()<<Q_FUNC_INFO<<m_parameter.parametersInfo();
