...
{
    d.data()->sampleRate = 0;
    d.data()->format = AudioFormat::PCM_S16LE;
    d.data()->sampleSize = 2;
    d.data()->validBitsPerSample = 16;
}

AudioParameters::AudioParameters(quint32 sampleRate, const ChannelMap &map, AudioParameters::AudioFormat f)
    : d(new AudioParametersPriv())
{
    d.data()->sampleRate = sampleRate;
    d.data()->channelMap = map;
    d.data()->format = f;
    d.data()->sampleSize = sampleSize(f);
    d.data()->validBitsPerSample = validBitsPerSample(f);
}
...
{
    return d.data()->channelMap.count();
}

const ChannelMap AudioParameters::channelMap() const
{
    return d.data()->channelMap;
}
...
return d.data()->sampleSize * d.data()->channelMap.count();
...
//    foreach (const auto  &it, d.data()->channelMap) {
//        list.append(names.value(it));
//    }

    return QString("SampleRate = [%1 Hz], Channels = [%2], AudioFormat = [%3]")
        .arg(d.data ()->sampleRate)
        .arg(d.data()->channelMap.toString())
        .arg (name);
}
